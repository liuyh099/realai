...
import cn.realai.online.core.bo.ExperimentResultSetBO;
import cn.realai.online.core.bo.ExperimentalResultQuatoBO;
import cn.realai.online.core.bo.ExperimentalResultTopBO;
...
public class ExperimentalResultController {
    ...
    public Result<ExperimentalResultTopVO> quotaTopGroup(@Validated IdVO idVo){
        try {
            ExperimentalResultTopBO experimentalResultTopVO=experimentalTrainBusiness.quotaTopGroup(idVo.getId());
        }catch (Exception e){
            logger.error("实验评估-图片异常",e);
            return new Result(ResultCode.DATA_ERROR.getCode(), ResultMessage.OPT_FAILURE.getMsg(), null);
        }
